...
function envelope(m) {
  ...
  return { id: m.id, from, to, task: id(m.task), kind: m.kind, revision: m.revision, summary: validateText(m.summary), references: validateReferences(m.references || []), supersedes: m.supersedes || null };
}
...
function insert(state, raw) {
  ...
  if (e.kind === 'assignment' && state.messages.some(m => m.envelope.kind === 'assignment' && m.envelope.to === e.to && !terminal.has(m.work))) throw Error('Recipient already has an active assignment; supersede it or wait for it to finish');
...
export function status(root, messageId, actor, work, revision, summary, references = []) {
  ...
  return mutate(root, state => {
    ...
    if (work !== 'working') {
      const notice = insert(state, { id: randomUUID(), from: actor, to: actor === m.envelope.from ? m.envelope.to : m.envelope.from, task: m.envelope.task, kind: 'notice', revision, summary: `${work}: ${messageId}. ${detail}`, references: refs });

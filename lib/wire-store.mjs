...
function envelope(m) {
  ...
  return { id: m.id, from, to, task: id(m.task), kind: m.kind, revision: m.revision, summary: validateText(m.summary), references: validateReferences(m.references || []), supersedes: m.supersedes || null, replyTo };
}

...
import { MAX_STATE_BYTES, transaction, read, digest, id, validateText, validateReferences, sessionFile, stateDir } from './store.mjs';
...
function insert(state, raw) {
  ...
  if (state.messages.length >= WIRE_CAPACITY) {
    ...
    throw Error(`Wire capacity reached (${state.messages.length}/${WIRE_CAPACITY}). ${assignments.length ? `Open assignments remain after archive: ${assignments.map(m => `${m.envelope.id} (${m.envelope.task})`).join(', ')}` : 'Run archive to clear.'}`);
  }
  if (e.supersedes) {
...
function archivedMessage(root, messageId) {
  ...
  for (const name of names) {
    const m = validateState(read(path.join(archiveDir(root), name))).messages.find(m => m.envelope.id === messageId);
    if (m) return m;
  }
  return null;
}
function beforeSend(root, state, e) {
  const canonical = envelope(e);
  ...
  if (state.messages.length >= WIRE_CAPACITY * 0.9 || Buffer.byteLength(JSON.stringify(state, null, 2) + '\n') >= MAX_STATE_BYTES * 0.9) archiveState(root, state, new Date());
  return null;
}
export const enqueue = (root, e) => mutate(root, state => beforeSend(root, state, e) || insert(state, e));

...
function archivedMessage(root, messageId) {
  ...
  for (const name of names) {
    ...
    if (m) return { ...m, archive: m.archive || { disposition: m.work, legacy: true } };
  }
  return null;
}
...
function findRetained(root, state, messageId) {
  if (state.messages.some(m => m.envelope.id === messageId)) return find(state, messageId);
  const archived = archivedMessage(root, messageId);
  if (!archived) throw Error(`Unknown wire message: ${messageId}`);
  return archived;
}
...
export function finishAttempt(root, messageId, attemptId, outcome) {
  ...
  return mutate(root, state => {
    const m = findRetained(root, state, messageId);
    if (m.archive && m.attempt?.id === attemptId) return m;
...
export function receive(root, messageId, actor, hash) {
  return mutate(root, state => {
    ...
    if (m.archive) return m; // a late wake must not resurrect archived work
...
export function operatorCancel(root, messageId, operator, reason) {
  ...
  return mutateLeases(root, leases => mutate(root, state => {
    ...
    try { if (fs.lstatSync(log).isSymbolicLink()) throw Error('Operator log is a link'); }
...
function archiveGate(root, state, now) {
  ...
  const endpointIsRecent = recipient => {
    ...
    let r = false;
    try {
      const s = read(sessionFile(root, provider, session));
      r = Boolean(s && (!Array.isArray(s.events) || s.events.some(e => !Number.isFinite(Date.parse(e?.at)) || Date.parse(e.at) >= cutoff)));
    } catch { r = true; } // unreadable evidence cannot prove inactivity
...
function writeArchive(root, name, state) {
  ...
  if (Buffer.byteLength(serialized) > MAX_READ_BYTES) throw Error('Archive capacity exceeded; live messages retained');

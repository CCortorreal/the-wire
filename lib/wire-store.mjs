...
function validateState(state) {
  ...
  for (const m of state.messages) if (digest(m.envelope) !== m.hash) throw Error('Wire envelope corrupted');
...
function find(state, messageId) {
  ...
  if (!m || digest(m.envelope) !== m.hash) throw Error('Unknown or corrupt wire message');
  return m;
}

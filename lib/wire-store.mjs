...
function envelope(m) {
  ...
  if (m.doneState != null) e.doneState = validateText(m.doneState);
  ...
  if (m.noticeReason != null) e.noticeReason = validateText(m.noticeReason);
...
function insert(state, raw) {
  ...
  if (state.messages.length >= WIRE_CAPACITY) {
    const assignments = state.messages.filter(m => m.envelope.kind === 'assignment' && !terminal.has(m.work));
    throw Error(`Wire capacity reached (${state.messages.length}/${WIRE_CAPACITY}). ${assignments.length ? `Outstanding assignments block archive: ${assignments.map(m => `${m.envelope.id} (${m.envelope.task})`).join(', ')}` : 'Run archive to clear.'}`);
  }
  if (e.supersedes) {
...
export function resubmit(root, sourceId, senderEndpoint, newEnvelope) {
  ...
  return mutate(root, state => {
    ...
    const inserted = insert(state, canonical);
...
export function operatorCancel(root, messageId, operator, reason) {
  ...
  const detail = validateText(reason);
  if (!detail.trim() || detail.length > MAX_STATUS) throw Error('Operator reason required within status limit');
...
export function archive(root, now = new Date()) {
  ...
  transaction(file(root), old => {
    ...
    if (gate.outstandingAssignments.length) throw Error(`Cannot archive: ${gate.outstandingAssignments.length} outstanding assignment(s): ${gate.outstandingAssignments.map(m => `${m.envelope.id} (${m.envelope.task}, ${m.work})`).join(', ')}`);
    if (gate.unsentNotices.length) throw Error(`Cannot archive: ${gate.unsentNotices.length} unsent notice(s): ${gate.unsentNotices.map(m => `${m.envelope.id} (${m.envelope.task})`).join(', ')}`);
    const at = now.toISOString();
    for (const m of gate.archivableOrphaned) { m.work = 'orphaned'; m.archive = { disposition: 'orphaned', reason: 'sender-inactive-24h', at }; }
    for (const m of gate.archivableUndelivered) m.archive = { disposition: 'undelivered', reason: 'recipient-inactive-24h', at };
    const target = writeArchive(root, `wire-${at.replace(/[-:.]/g, '')}-${randomUUID().slice(0, 8)}.json`, state);
    result = { schema: 'the-wire-archive-result/v1', archive: path.relative(root, target).split(path.sep).join('/'), messages: state.messages.length, undelivered: gate.archivableUndelivered.map(m => m.envelope.id), orphaned: gate.archivableOrphaned.map(m => m.envelope.id), sequences: { ...state.sequences } };

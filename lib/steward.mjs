...
export function sweep(root, opts = {}) {
  ...
  for (const m of list(root)) {
    ...
    if (m.delivery === 'unconfirmed' || m.delivery === 'attempting') {
      ...
      if (prior.length >= MAX_REWAKES) continue;
      if (prior.length > 0 && Date.now() - new Date(prior.at(-1).ts).getTime() < BACKOFF_MS[Math.min(prior.length - 1, BACKOFF_MS.length - 1)]) continue;
      let ok = false;
